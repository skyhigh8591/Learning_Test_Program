;PROGRAM : U1_2_1.ASM 							2017.0413
;FUNCTION: 7 SEGMENT LED DEMO PROGRAM(ITABRD)	By Steven   
...
MAIN:         
		MOV    	A,HIGH (TAB_7_SEG-1) 
		MOV  	TBHP,A    				;取得建表起始位址(High-byte) 
		MOV    	A,LOW  (TAB_7_SEG-1) 
		MOV  	TBLP,A    				;取得建表起始位址(Low-byte)  
		MOV    	A,10 
		MOV    	COUNT,A      			;設定迴圈次數 
LOOP: 
		ITABRD	SEG_PORT     			;取得建表資料，並將Low-byte送至SEG_PORT 
		MOV  	A,TBLH   				;取得High-byte資料 
		CALL    DELAY    				;延遲ACC*10ms 
		SDZ    	COUNT      				;(COUNT=COUNT-1)是否為0?
		JMP  	LOOP    				;否，迴圈繼續 
		JMP  	MAIN    				;是，重新執行
;========================================================================================
; PROC	: DELAY

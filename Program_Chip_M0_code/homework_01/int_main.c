...
extern void  colorLed_main (void);
...
{       
    msTicks++;           // Increment Counter  每 1ms 中斷後 增加 1
}
...
{
	RCC->AHBENR |= 0x1ul << 17;                 // Enable ... clock 
	GPIOA->MODER &= ~( 0x3ul << 9*2 | 0x3ul << 10*2);   // Set PA9、PA10 is input
	...
	EXTI->FTSR |= 0x1ul << 9 | 0x1ul << 10;          //Falling trigger selection register
	
	EXTI->IMR |= 0x1ul << 9 | 0x1ul << 10;           //Interrup mask register

	EXTI->PR |= 0x1ul << 9 | 0x1ul << 10;	          //pending register
...
//-------------------------------------------
void Config_PB4PB6(void)
{
	RCC->AHBENR |= 0x1ul << 18;                 // Enable GPIOB clock 
	GPIOB->MODER &= ~( 0x3ul << 4*2 | 0x3ul << 6*2 );   // Set PB4、PB6 is input
	
	RCC->APB2ENR |= 0x1ul;                      // Enable SYSCFG
		
	SYSCFG ->EXTICR[1] &= ~( 0xful  );
	SYSCFG ->EXTICR[1] |= 0x1ul ;

	SYSCFG ->EXTICR[1] &= ~( 0xful << 2*4 );
	SYSCFG ->EXTICR[1] |= 0x1ul << 2*4;	
	
	EXTI->FTSR |= 0x1ul << 4 | 0x1ul << 6;          //Falling trigger selection register
	EXTI->IMR |= 0x1ul << 4 | 0x1ul << 6;           //Interrup mask register
	EXTI->PR |= 0x1ul << 4 | 0x1ul << 6;	          //pending register
...
//---------PB4-
	if ((prValue & (0x1ul << 4)) == (0x1ul <<4 ) )
	{
		if ((GPIOB->IDR & (0x1ul << 4) ) == 0)
		{
			key_number = Key_5;            //中斷只記錄，處理工作放在主程式
			printf("PB.4 int.\n\r");
		}
	}	
	EXTI->PR |= 0x1ul << 4;                 //中斷一定要清除
//---------PB6
	if ((prValue & (0x1ul << 6)) == (0x1ul <<6 ) )
	{
		if ((GPIOB->IDR & (0x1ul << 6) ) == 0)
		{
			key_number = Key_6;            //中斷只記錄，處理工作放在主程式
			printf("PB.6 int.\n\r");
		}
	}	
	EXTI->PR |= 0x1ul << 6;                 //中斷一定要清除
//---------PA9
	if ((prValue & (0x1ul << 9)) == (0x1ul <<9 ) )
	{
		if ((GPIOA->IDR & (0x1ul << 9) ) == 0)
		{
			key_number = Key_7;            //中斷只記錄，處理工作放在主程式
			printf("PA.9 int.\n\r");
		}
	}	
	EXTI->PR |= 0x1ul << 9;                 //中斷一定要清除
//---------PA10
	if ((prValue & (0x1ul << 10)) == (0x1ul <<10 ) )
	{
		if ((GPIOA->IDR & (0x1ul << 10) ) == 0)
		{
			key_number = Key_8;            //中斷只記錄，處理工作放在主程式
			printf("PA.10 int.\n\r");
		}

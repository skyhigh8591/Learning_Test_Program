   ...
   for(repeat=0;repeat<20;repeat++)   //重覆掃瞄次數
    {   
      for(i=0;i<127;i=i+2) //讀取四個字型資料分偶數及奇數
	   ...
	   if (i<=31)          LED_CTR=~(1<<0);//11111110若第一字型，P20=0  
	   if (i>=32 && i<=63) LED_CTR=~(1<<1);//若第二字型，P21=0
	   if (i>=64 && i<=95) LED_CTR=~(1<<2);//若第三字型，P22=0
	   if (i>=96)          LED_CTR=~(1<<3);//若第四字型，P23=0 
   	  }
   }
  color++;	//換下一種彩色
